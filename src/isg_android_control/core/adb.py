...
class ADBController:
    ...
    async def set_volume_percent(self, percent: int) -> None:
        ...
        if maxv:
            ...
            # Try settings method first (more compatible)
            try:
                await self._run("shell", "settings", "put", "system", "volume_music", str(lvl))
                logger.debug("Volume set successfully via settings command")
                return
            except Exception as e:
                logger.debug("Failed to set volume via settings command: %s", e)
            # Try modern media volume command
            try:
                await self._run("shell", "media", "volume", "--stream", "3", "--set", str(lvl))
                logger.debug("Volume set successfully via media command")
                return
            except Exception as e:
                logger.debug("Failed to set volume via media command: %s", e)
        ...
        if cur is None or maxv is None:
            ...
            logger.info("Using keyevent approximation for volume control (%d steps %s)", steps, direction)
            ...
            logger.info("Using keyevent fallback for volume control (%d steps %s)", abs(delta), cmd)
    ...
    async def set_volume_index(self, index: int) -> None:
        ...
        cur = info.get("current")
        if cur is None:
            logger.warning("Cannot set volume: unable to get current volume level")
            return
    ...
    async def screen_state(self) -> bool:
        # Use the specific mScreenState field from dumpsys display for most accurate state
        try:
            state = await self._run("shell", "dumpsys", "display", timeout=8.0)
            if "mScreenState=" in state:
                # Extract the exact mScreenState value
                for line in state.splitlines():
                    if "mScreenState=" in line:
                        return "mScreenState=ON" in line
                # Fallback to simple search if line parsing fails
                return "mScreenState=ON" in state
        except Exception as e:
            logger.debug("Failed to get screen state via dumpsys display: %s", e)
        
        # Fallback to dumpsys power method
        try:
            ...
            logger.warning("Failed to get screen state via both display and power dumpsys: %s", e)
            return False
